...
class VpRouteSearchableDropdown extends StatelessWidget {
  final LaneDetailsResponse? selectedRoute;
  final ValueChanged<LaneDetailsResponse?> onChanged;
  final String labelText;
  final String hintText;
  final bool mandatoryStar;

  final Future<List<LaneDetailsResponse>> Function(int page, String? searchKey)fetchRoutes;

  VpRouteSearchableDropdown({
    super.key,
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: SearchableDropdown<LaneDetailsResponse>.paginated(
            ...
            initialValue:
            selectedRoute != null
                ? SearchableDropdownMenuItem<LaneDetailsResponse>(
              value: selectedRoute!,
              label:
              "${selectedRoute?.lane}",
              child: Text(
            ...
            paginatedRequest: (int page, String? searchKey) async {
              ...
              if ((cubit.state.profileDetailUIState?.data?.customer?.laneDetails??[]).isEmpty) {
                return [];
              }
            final allLanes=  cubit.state.profileDetailUIState?.data?.customer?.laneDetails??[];
              return allLanes.map((route) {

                return SearchableDropdownMenuItem<LaneDetailsResponse>(
                  value: route,
                  label: route.lane??"",
                  child: Text(route.lane??""),
                );
              }).toList();
            },
