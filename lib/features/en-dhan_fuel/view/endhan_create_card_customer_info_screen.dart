...
class _EndhanCreateCardCustomerInfoScreenState extends State<EndhanCreateCardCustomerInfoScreen> {
  ...
  final List<DropdownMenuItem<String>> titleItems = const [
  ...
  String? _getValidRegionalOfficeValue(EnDhanState state) {
    ...
    final validOffices = state.regionalOffices.where((regional) => 
      (regional['id'] ?? '').toString() == selectedId
    ).toList();
  ...
  String? _getValidDistrictValue(EnDhanState state) {
    ...
    final validDistricts = state.districts.where((district) => 
      (district['id'] ?? '').toString() == selectedId
    ).toList();
               ...
               // padding: EdgeInsets.all(20),
                children: [
                  ...
                  Row(
                      ...
                      Expanded(
                        ...
                        child: AppDropdown(
                         // labelText: 'Title',
                          ...
                          dropDownList: titleItems,
                          onChanged: (val) {
                            setState(() {
                              selectedTitle = val;
                            });
                            // Update cubit with selected title
                            if (val != null) {
                              cubit.setTitle(val);
                            }
                          },
                    ...
                    //labelText: 'Name',
                    hintText: 'Enter name',
                    onChanged: (val) => cubit.setCustomerName(val),
                    validator: (value) {
                  ...
                  AppDropdown(
                    ...
                    dropdownValue: state.selectedZonalOfficeId?.toString(),
                    dropDownList: state.zonalOffices.isNotEmpty
                        ? state.zonalOffices.map((zonal) => 
                          DropdownMenuItem(
                            value: (zonal['id'] ?? '').toString(),
                            child: Text(zonal['zone_name'] ?? ''),
                          )
                        ).toList()
                        : [DropdownMenuItem(value: '', child: Text('Loading zonal offices...'))],
                    onChanged: (val) {
                  ...
                  AppDropdown(
                    ...
                    dropDownList: state.regionalOffices.isNotEmpty
                        ? state.regionalOffices.map((regional) => 
                          DropdownMenuItem(
                            value: (regional['id'] ?? '').toString(),
                            child: Text(regional['region_name'] ?? ''),
                          )
                        ).toSet().toList() // Remove duplicates
                        : [DropdownMenuItem(value: '', child: Text('Select zonal office first'))],
                    onChanged: (val) {
                      if (val != null && val.isNotEmpty) {
                  ...
                  AppDropdown(
                    ...
                    dropDownList: state.states.isNotEmpty 
                        ? state.states
                            .map<DropdownMenuItem<String>>((stateItem) => DropdownMenuItem<String>(
                                  value: (stateItem['id'] ?? '').toString(),
                                  child: Text(stateItem['name'] ?? ''),
                                ))
                            .toList()
                        : [DropdownMenuItem(value: '', child: Text('Loading states...'))],
                    onChanged: (val) {
                      if (val != null && val.isNotEmpty) {
                  ...
                  AppDropdown(
                    ...
                    dropDownList: state.districts.isNotEmpty
                        ? state.districts
                            ...
                            .toSet().toList() // Remove duplicates
                        : [DropdownMenuItem(value: '', child: Text('Select state first'))],
                    onChanged: (val) {
                      if (val != null && val.isNotEmpty) {
                  ...
                  AppTextField(
                    ...
                    onChanged: (val) => cubit.setCommunicationCityName(val),
                    validator: (value) {
                      if (value == null || value.trim().isEmpty) {
                      ...
                      if (_formKey.currentState?.validate() ?? false) {
                        ...
                        if (state.selectedZonalOfficeId == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Please select a zonal office')),
                          );
                          return;
                        }
                        if (state.selectedRegionalOfficeId == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Please select a regional office')),
                          );
                          return;
                        }
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (_) => EndhanCreateCardInfoScreen()),
                        );
                      }
                    },
                  ),
