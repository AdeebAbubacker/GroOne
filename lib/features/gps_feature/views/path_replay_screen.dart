...
class PathReplayScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => context.read<PathReplayCubit>()
        ..fetchPathReplay(token, queryParams),
      child: PathReplayView(
        vehicleNumber: vehicleNumber ?? queryParams['vehicleNumber']?.toString() ?? '',
      ),
    );
  }
}
...
class PathReplayView extends StatelessWidget {
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, PathReplayState state) {
    return AppBar(
      leading: IconButton(
  ...
  Widget _buildBody(BuildContext context, PathReplayState state) {
    ...
    if (state.pathPositions.isEmpty) {
      return const Center(
  ...
  Widget _buildAddressOverlay(BuildContext context, PathReplayState state) {
    if (state.pathPositions.isEmpty) return const SizedBox.shrink();

    final currentIndex = state.currentIndex.clamp(0, state.pathPositions.length - 1);
  ...
  Widget _buildBottomStatsPanel(BuildContext context, PathReplayState state) {
    ...
    final currentIndex = state.currentIndex.clamp(0, state.pathPositions.length - 1);
    final currentSpeed = state.pathPositions[currentIndex].speed ?? 0;
    final currentDistance = state.pathPositions[currentIndex].totalDistance ?? 0;
    final totalDistance = state.pathPositions.last.totalDistance ?? 0;
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: state.pathPositions.isNotEmpty
                        ? SliderTheme(
                            ...
                            child: Slider(
                              ...
                              min: 0,
                              max: (state.pathPositions.length - 1).toDouble(),
                              divisions: state.pathPositions.length > 1
                                  ? state.pathPositions.length - 1
                                  : 1,
                              onChanged: (value) {
                                context.read<PathReplayCubit>().seek(value.toInt());
                              },
                            ),
                          )
...
class _PathReplayMapWidgetState extends State<PathReplayMapWidget> {
  ...
  Widget _buildMap() {
    final pathPoints = widget.state.pathPositions
        .map((pos) => LatLng(pos.latitude!, pos.longitude!))
        .toList();

    final currentIndex = widget.state.currentIndex.clamp(0, pathPoints.length - 1);
    final currentPosition = pathPoints.isNotEmpty ? pathPoints[currentIndex] : null;

    final markers = <Marker>{};
    if (currentPosition != null && widget.state.truckIcon != null) {
      final cubit = context.read<PathReplayCubit>();
      markers.add(
        Marker(
          ...
          infoWindow: InfoWindow(
            ...
            snippet: "Speed: ${widget.state.pathPositions[currentIndex].speed?.toStringAsFixed(1) ?? 0} Km/h",
          ),
        ),
      );
    }
  ...
  void _handleMapCreated(GoogleMapController controller) {
    ...
    if (!widget.state.hasFitToPath && widget.state.pathPositions.length > 1) {
  ...
  void _handleStateChange(PathReplayState state) async {
    ...
    // Follow truck during playback
    if (state.hasFitToPath && 
        state.animatedMarkerPosition != null && 
        state.isPlaying) {
      controller.animateCamera(CameraUpdate.newLatLng(state.animatedMarkerPosition!));
    }
  }
}
