...
class _GpsOrderSummaryScreenState extends State<GpsOrderSummaryScreen> {
  ...
  Future<void> _fetchOrderSummary() async {
    try {
      ...
      final products = widget.products.map((product) {
        // Get state from billing address
        final state = widget.billingAddress.state.isNotEmpty 
      ...
      final request = GpsOrderSummaryRequest(
        products: products,
      );
    ...
    return BlocListener<GpsOrderCubit, GpsOrderState>(
      ...
      listener: (context, state) {
        ...
        if (state is GpsOrderSummaryLoaded) {
          ...
          ToastMessages.error(message: "Failed to load order summary: ${state.message}");
        }
      },
      child: Scaffold(
      appBar: CommonAppBar(
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            Text(context.appText.paymentDetails, style: AppTextStyle.h4),
            10.height,
            Text('Failed to load order summary', style: AppTextStyle.textDarkGreyColor14w500),
          ],
        ),
      );
    }
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(summaryItem.partName, style: AppTextStyle.textDarkGreyColor14w500),
                        ],
                      ),
                    ),
                    Text("₹${(summaryItem.unitPrice * summaryItem.quantity)}", style: AppTextStyle.blackColor15w500),
                  ],
                ),
                5.height,
                _buildDetailRow("HSN Code", summaryItem.hsnCode),
                _buildDetailRow("Qty", summaryItem.quantity.toString().padLeft(2, '0')),
                _buildDetailRow("Rate /Unit ₹", "₹${summaryItem.unitPrice}"),
                if (summaryItem.discount > 0) _buildDetailRow("Discount", "₹${summaryItem.discount}"),
  ...
  Map<String, String> _parseAddress(String addressString) {
    ...
    print('Parsed address - City: $city, State: $state, PostalCode: $postalCode');
  ...
  Future<void> _createGpsOrder() async {
    try {
      ...
      print('Billing address - Original: city=${widget.billingAddress.city}, state=${widget.billingAddress.state}, pincode=${widget.billingAddress.pincode}');
      print('Billing address - Parsed: city=${billingAddressParts['city']}, state=${billingAddressParts['state']}, postalCode=${billingAddressParts['postalCode']}');
      final billingAddress = GpsOrderAddress(
        ...
        city: widget.billingAddress.city.isNotEmpty ? widget.billingAddress.city : billingAddressParts['city']!,
        state: widget.billingAddress.state.isNotEmpty ? widget.billingAddress.state : billingAddressParts['state']!,
        postalCode: widget.billingAddress.pincode.isNotEmpty ? widget.billingAddress.pincode : billingAddressParts['postalCode']!,
      ...
      print('Shipping address - Original: city=${widget.shippingAddress.city}, state=${widget.shippingAddress.state}, pincode=${widget.shippingAddress.pincode}');
      print('Shipping address - Parsed: city=${shippingAddressParts['city']}, state=${shippingAddressParts['state']}, postalCode=${shippingAddressParts['postalCode']}');
      final shippingAddress = GpsOrderAddress(
        ...
        city: widget.shippingAddress.city.isNotEmpty ? widget.shippingAddress.city : shippingAddressParts['city']!,
        state: widget.shippingAddress.state.isNotEmpty ? widget.shippingAddress.state : shippingAddressParts['state']!,
        postalCode: widget.shippingAddress.pincode.isNotEmpty ? widget.shippingAddress.pincode : shippingAddressParts['postalCode']!,
        country: widget.shippingAddress.country,
        gstId: widget.shippingAddress.gstin ?? "",
      );
      
      // Create order items
      final orders = widget.products.map((product) {
        ...
        return GpsOrderItem(
          ...
          vehicleNumbers: vehicleNumbers.map((v) => GpsOrderVehicle(vehicleNumber: v)).toList(),
        );
      }).toList();
      ...
      final request = GpsOrderRequest(
        ...
        totalPrice: orderSummary?.data.grandTotal ?? _fallbackTotalAmount, // Use the API's grandTotal or fallback
        categoryId: 1,
