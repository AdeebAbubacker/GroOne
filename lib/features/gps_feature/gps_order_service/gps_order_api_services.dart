  ...
  Future<Result<List<KavachUserModel>>> fetchUsers({
    ...
    try {
      ...
      if (response is Success) {
        CustomLog.debug(this, "GPS Fetch Users - Raw Response: ${response.value}");
        ...
        try {
          ...
          if (responseData is Map<String, dynamic>) {
            ...
            if (responseData.containsKey('data') && responseData['data'] is List) {
              ...
              CustomLog.debug(this, "GPS Fetch Users - Parsed successfully: ${userListResponse.data.length} users");
              return Success(userListResponse.data);
            } else if (responseData.containsKey('success') || responseData.containsKey('status')) {
              // Fallback to getResponseStatus if response has success/status fields
              return await _apiService.getResponseStatus(
                response.value,
                (data) {
                  try {
              ...
              try {
                final users = (responseData['data'] as List)
                    ...
                    .toList();
                CustomLog.debug(this, "GPS Fetch Users - Parsed as direct list: ${users.length} users");
            ...
            CustomLog.error(this, "GPS Fetch Users - Invalid response format", null);
            return Error(DeserializationError());
          }
        } catch (e) {
          CustomLog.error(this, "GPS Fetch Users - Failed to parse response", e);
          return Error(DeserializationError());
        }
      } else {
        CustomLog.error(this, "GPS Fetch Users - API call failed: ${response.runtimeType}", null);
