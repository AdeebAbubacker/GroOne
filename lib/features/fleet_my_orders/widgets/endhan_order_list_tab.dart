...
class EndhanOrderListTabWidget extends StatefulWidget {
  ...
  @override
  State<EndhanOrderListTabWidget> createState() => _EndhanOrderListTabWidgetState();
}
            ...
            child: BlocBuilder<EnDhanCubit, EnDhanState>(
              builder: (context, state) {
                ...
                final filtered = cards.where((c) {
                  return (c.cardNumber ?? '').toLowerCase().contains(_searchText) ||
                      (c.vehicleNumber ?? '').toLowerCase().contains(_searchText);
                }).toList();
